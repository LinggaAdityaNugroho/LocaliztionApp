...
const DetailPeminjamanModal: React.FC<ModalProps> = ({
  isOpen,
  onClose,
  data,
}) => {
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              {data.details?.map((det: any) => (
                <div
                  key={det.id}
                  className="bg-slate-50 p-4 rounded-3xl border border-slate-100 hover:border-indigo-200 transition-colors"
                >
                      ...
                      <div className="mt-2 flex flex-wrap gap-1">
                        ...
                        {det.kode_tag_list ? (
                          det.kode_tag_list.map((tag: string) => (
                            <span
                              key={tag}
                              className="text-[8px] bg-indigo-600 text-white px-2 py-0.5 rounded-md font-black tracking-tighter"
                            >
                              {tag}
                            </span>
                          ))
          ...
          <div className="grid grid-cols-2 gap-4 text-center bg-slate-50 p-4 rounded-3xl border border-slate-100">
            <div>
              ...
              <p className="font-black text-slate-800 italic uppercase text-sm leading-none mt-1">
                {data.waktu_pinjam
                  ? new Date(data.waktu_pinjam).toLocaleTimeString("id-ID", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })
                  : "-"}
